(function (window) {
    ...
    c3.generate = function (config) {
        ...
        // arc
        var __arc_label_fomat = getConfig(['arc', 'label', 'format'], function (d, ratio) {
        ...
        var __tooltip_enabled = getConfig(['tooltip', 'enabled'], true),
            __tooltip_contents = getConfig(['tooltip', 'contents'], function (d) {
                var yFormat = __axis_y_tick_format ? __axis_y_tick_format : function (v) {
                        return +v;
                    },
                    xFormat = getXAxisTickFormat(),
                    text, i, title, value, name;
                for (i = 0; i < d.length; i++) {
                    if (!d[i] || !isValue(d[i].value)) {
                        continue;
                    }
        ...
        function updateScales() {
            ...
            // update scales
            x = getX(xMin, xMax, x ? x.domain() : undefined, function () {
        ...
        function expandArc(id, withoutFadeOut) {
            var target = svg.selectAll('.chart-arc' + getTargetSelector(id)),
                noneTargets = svg.selectAll('.-arc').filter(function (data) {
        ...
        function getXDomainMax(targets) {
            return d3.max(targets, function (t) {
        ...
        function convertDataToTargets(data) {
            ...
            if (isCustomX) {
                ids.forEach(function (id) {
                    ...
                    if (xs.indexOf(xKey) >= 0) {
                        c3.data.x[id] = data.map(function (d) {
                            return d[xKey];
                        });
            ...
            // set target types
            if (__data_type) {
                setTargetType(getTargetIds(targets).filter(function (id) {
        ...
        function classArc(d) {
            return classShapes(d.data) + " -arc -arc-" + d.data.id;
        }

        function classArea(d) {
            return classShapes(d) + " -area -area-" + d.id;
        }
        ...
        */
        ...
        function parseDate(date) {
            var parsedDate;
            if (!date) {
                throw Error(date + " can not be parsed as d3.time with format " + __data_x_format + ". Maybe 'x' of this data is not defined. See data.x or data.xs option.");
            }
            parsedDate = d3.time.format(__data_x_format).parse(date);
            if (!parsedDate) {
                throw Error("Failed to parse '" + date + "' with format " + __data_x_format);
            }
            return parsedDate;
        }
        ...
        var lineOnSub = (function () {
            var line = d3.svg.line()
                ...
                .y(__axis_rotated ? subxx : function (d) {
                    return getSubYScale(d.id)(d.value);
                });
            return function (d) {
        ...
        }).on("zoom", __zoom_enabled || __scroll_enabled ? redrawForZoom : null);
        ...
        function init(data) {
            ...
            brush.scale(subX);
            if (__zoom_enabled || __scroll_enabled) {
                zoom.scale(x);
            }
        ...
        function drag(mouse) {
            ...
            if ((__zoom_enabled || __scroll_enabled) && !zoom.altDomain) {
                return;
        ...
        function redraw(options) {
            ...
            if (__grid_x_show) {
                ...
                xgrid.attr("x1", __axis_rotated ? 0 : function (d) {
                    return x(d) - xAxis.tickOffset();
                })
                    .attr("x2", __axis_rotated ? width : function (d) {
                        return x(d) - xAxis.tickOffset();
                    })
                    .attr("y1", __axis_rotated ? function (d) {
                        return x(d) - xAxis.tickOffset();
                    } : margin.top)
                    .attr("y2", __axis_rotated ? function (d) {
                        return x(d) - xAxis.tickOffset();
                    } : height)
        ...
        function updateTargets(targets) {
            ...
            mainPieEnter = mainPieUpdate.enter().append("g")
                .attr("class", function (d) {
                    return 'chart-arc target target-' + d.data.id;
                });
        ...
        c3.data.getAsTarget = function (targetId) {
            var targets = getTargets(function (t) {
                return t.id === targetId;
            });
    ...
    function categoryAxis() {
        ...
        var tickMajorSize = 6, /*tickMinorSize = 6,*/ tickEndSize = 6, tickPadding = 3, tickCentered = false,
            tickTextNum = 10, tickOffset = 0, tickFormat = null;
        var categories = [];
                ...
                switch (orient) {
                    case "bottom": {
                        ...
                        text.text(function (i) {
                            return shouldShowTickText(ticks, i) ? formattedCategory(i) : "";
                        });
